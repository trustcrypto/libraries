...
static const BYTE_ charset[]={"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"};

/*********************** FUNCTION DEFINITIONS ***********************/
BYTE_ revchar(char ch)
{
	if (ch >= 'A' && ch <= 'Z')
		ch -= 'A';
	else if (ch >= 'a' && ch <='z')
...
size_t base64_encode(const BYTE_ in[], BYTE_ out[], size_t len, int newline_flag)
{
	...
	if (out == NULL) {
		...
		if (left_over)
			idx2 += 4;
		if (newline_flag)
			idx2 += len / 57;   // (NEWLINE_INVL / 4) * 3 = 57. One newline per 57 input BYTE_s.
	}
	else {
		// Since 3 input BYTE_s = 4 output BYTE_s, determine out how many even sets of
		// 3 BYTE_s the input has.
		blk_ceiling = blks * 3;
...
size_t base64_decode(const BYTE_ in[], BYTE_ out[], size_t len)
{
	BYTE_ ch;
